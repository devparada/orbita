...
export default function Meteorites() {
	...
	useEffect(() => {
		...
		getMeteoritos().then((data) => {
			...
			const formatted = data.slice(0, 20).map((m, i) => {
				...
				// Radio máximo de 1.2 para que sean siempre más pequeños que la Luna (1.36)
				const rad = Math.max(0.3, Math.min(1.2, (m.diameterMax / 1000) * 1.5));
				...
				targets.forEach((t) => {
					...
					if (d < b1.radius + t.pk) {
						...
						setParticles((p) => [
							...
							{
								id: Math.random(),
								pos: new THREE.Vector3(b1.pos.x, 0, b1.pos.z),
								color: t.c,
							},
						]);
						setLabels((l) => [
	...
	return (
		<group ref={sceneRef}>
			...
			{particles.map((p) => (
				<ImpactParticles key={p.id} position={p.pos} color={p.color} />
			))}
			{labels.map((l) => (
