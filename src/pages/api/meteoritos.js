...
export async function GET() {
  ...
  try {
    ...
    if (!response.ok) {
      return new Response(JSON.stringify({ error: 'Error de la NASA' }), { status: response.status });
    }

    const data = await response.json();

    // Extraemos y aplanamos los objetos
    const rawObjects = data.near_earth_objects ? Object.values(data.near_earth_objects).flat() : [];
    ...
    const cleanData = rawObjects.map(m => {
      ...
      return {
        ...
        velocity: parseFloat(firstApproach?.relative_velocity?.kilometers_per_hour || "0"),
        date: firstApproach?.close_approach_date || "",
        missDistance: parseFloat(firstApproach?.miss_distance?.kilometers || "0"),
      };
    });
